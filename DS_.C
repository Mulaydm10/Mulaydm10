...
// Structure to store information about each medicine
typedef struct {
    char name[50];
    int quantity;
    float price;
...
// Function to pop a medicine from the stack
...
// Function to display the stock of a specific medicine type
...
int main() {
    ...
    if (role == 1) {
        ...
        // Display available medicine types
        Node* current = head;
        while (current != NULL) {
            printf("%s\n", current->type);
            current = current->next;
    ...
    // Free memory for the nodes in the database
    Node* current = head;
    while (current != NULL) {
        Node* temp = current;
        current = current->next;
